...
function M:doc2pdf(job)
	...
	--[[	4. Only one LibreOffice process may use a given user profile at a time. Yazi
	  runs the preloader and the previewer concurrently, so without a dedicated
	  profile the second process silently converts nothing and still exits 0.
	  A fixed profile path keeps it warm across previews; the retry below covers
	  the case where the two jobs still overlap on it. --]]
	local profile = "-env:UserInstallation=file://" .. tmp .. "profile"
